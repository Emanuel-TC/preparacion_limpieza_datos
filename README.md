...
6. **Evaluación Final (Ajuste del Modelo):** Se evaluó el ajuste del algoritmo mediante la Curva ROC y la distribución de densidades de probabilidades predichas. El AUC confirma la capacidad predictiva del modelo, aunque la superposición de densidades entre ambas clases evidencia el límite de aprendizaje de los modelos lineales (Regresión Logística) frente a comportamientos financieros complejos. Esto justifica la futura exploración de modelos basados en árboles (ej. XGBoost) para maximizar la separación de clases en el departamento de Riesgos.